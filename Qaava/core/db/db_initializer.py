...
from ...core.exceptions import QaavaDatabaseError, QaavaInitializationCancelled
...
class DatabaseInitializer:
    ...
    def initialize_database(self, db_conn_name: str, plan_sting: str) -> None:
        ...
        try:
            ...
            try:
                ...
                LOGGER.warning(
                    tr("Not implemented:"),
                    extra=bar_msg(tr("Project SQL is not implemented yet")),
                )
            ...
            raise QgsPluginNetworkException(
                ...
                bar_msg=bar_msg(
                    tr(
                        "Could not load schema for the plan. Check your internet connection. Details: "
                    )
                    + str(e)
                ),
            )
        ...
        try:
            ...
            raise QaavaDatabaseError(
                tr("Error occured while inserting the project"),
                bar_msg(
                    tr(
    ...
    def register_database(self, db_conn_name: str, plan_sting: str) -> None:
        ...
        self.plan: LandUsePlan = (
            self.plan_enum.value()
        )  # intance of plan class because of ()
        ...
        if self.conn_params["user"] is None or self.conn_params["password"] is None:
            # TODO: here we ask for credentials, but user/password may still be None
            ask_credentials_dlg = DbAskCredentialsDialog(
                self.conn_params["user"], self.conn_params["password"]
            )
            result = ask_credentials_dlg.exec_()
            if result:
                self.conn_params[
                    "user"
                ] = ask_credentials_dlg.userLineEdit.text()  # may be None
                self.conn_params[
                    "password"
                ] = ask_credentials_dlg.pwdLineEdit.text()  # may be None
            else:
                raise QaavaInitializationCancelled(
                    tr("Canceling"),
                    bar_msg=bar_msg(
                        tr(
                            "Could not continue initializing without username and password"
                        )
                    ),
                )
        ...
        if not self.database.is_valid():
            raise QaavaDatabaseError(
                ...
                bar_msg=bar_msg(tr("Could not access the database.")),
            )
    ...
    def promote_database(self):
        ...
        current_version, newest_version = (
            version_from_string(v) for v in self.get_versions()
        )
        ...
        if current_version < newest_version:
            try:
                ...
                raise QaavaDatabaseError(
                    tr("Error occurred while running migration script"),
                    bar_msg(
                        tr(
    ...
    def get_versions(self) -> Tuple[str, str]:
        ...
        try:
            ...
            current_version = max(
                [version_from_string(v[0]) for v in self.database.execute_select(query)]
            )
            LOGGER.debug(f"Database version is {current_version}")
        except psycopg2.OperationalError:
            raise QaavaDatabaseError(
                tr("Connection error"), bar_msg(tr("Could not connect to the database"))
            )
        except psycopg2.DatabaseError:
            raise QaavaDatabaseError(
                tr("The database is not initialized properly"),
                bar_msg(tr("Please initialize the database")),
            )
        return string_from_version(current_version), string_from_version(
            self.plan.newest_version
        )
    ...
    def get_available_projects(self) -> List[str]:
        ...
        try:
            ...
            LOGGER.warning(
                tr("No projects available:"),
                extra=bar_msg(tr("No projects found from the database")),
            )
            available_projects = []
