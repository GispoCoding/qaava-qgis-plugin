...
from ...core.db.qgis_project_utils import (
    fix_data_sources_from_binary_projects,
    fix_project,
    load_project,
)
from ...model.land_use_plan import LandUsePlanEnum
from ..conftest import IFACE, get_test_resource
...
def test_fix_data_sources_from_binary_projects(database_params):
    ...
    with open(get_test_resource("test_data", "general_project.qgz"), "rb") as f:
        content = f.read()
    fixed = fix_data_sources_from_binary_projects(
        database_params, "test_auth_cfg", [content], _test=True
    )[0]
    assert fixed.count(conn_string) == 241


@pytest.mark.skip(
    "opening any project fails in test environment... Is there a solution for this?"
)
